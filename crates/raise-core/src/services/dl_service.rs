// FICHIER : src-tauri/src/services/dl_service.rs
...
pub fn init_dl_model(state: &DlState, config: DeepLearningConfig) -> RaiseResult<String> {
...
pub fn run_dl_prediction(state: &DlState, input: Vec<f32>) -> RaiseResult<Vec<f32>> {
...
pub fn train_dl_step(state: &DlState, input: Vec<f32>, target: u32) -> RaiseResult<f64> {
    let device = AppConfig::device();
...
pub fn save_dl_model(state: &DlState, path: String) -> RaiseResult<String> {
    let vg = match state.varmap.lock() {
        Ok(g) => g,
        Err(_) => raise_error!("ERR_SYS_MUTEX_POISONED"),
    };
...
mod tests {
    ...
    async fn test_dl_commands_initialization() -> RaiseResult<()> {
        ...
        let res = init_dl_model(&state, config);
        assert!(res.is_ok());
        Ok(())
    }
    ...
    async fn test_resilience_uninitialized_prediction() -> RaiseResult<()> {
        ...
        let res = run_dl_prediction(&state, vec![0.1]);
        assert!(res.is_err());
        Ok(())
    }
