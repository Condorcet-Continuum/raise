// FICHIER : src-tauri/src/services/json_db_service.rs
...
pub async fn jsondb_create_db(storage: &StorageEngine, space: &str, db: &str) -> RaiseResult<bool> {
...
pub async fn jsondb_drop_db(storage: &StorageEngine, space: &str, db: &str) -> RaiseResult<bool> {
...
pub async fn jsondb_update_document(
    ...
    match manager.update_document(collection, id, document).await {
        ...
        Ok(_) => Ok(json_value!({
            "status": "success",
            "message": format!("Document {} mis à jour dans {}", id, collection)
        })),
        Err(e) => raise_error!(
