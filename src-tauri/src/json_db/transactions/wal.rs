...
pub async fn write_entry(
    ...
    fs::write_json_atomic(&file_path, &log).await?;
...
pub async fn list_pending(config: &JsonDbConfig, space: &str, db: &str) -> AnyResult<Vec<String>> {
    let dir = get_wal_dir(config, space, db);
...
mod tests {
    ...
    async fn test_wal_persistence() {
        ...
        if !fs::exists(&wal_path).await {
            fs::ensure_dir(&wal_path).await.unwrap();
        }
